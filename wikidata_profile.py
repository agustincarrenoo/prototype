...
def render_graph_html(item_id: str, properties, top_n_properties: int = 8, max_values_per_property: int = 3) -> str:
    if not properties:
        return "<p>No graph data available.</p>"

    sorted_props = sorted(properties.items(), key=lambda kv: len(kv[1]), reverse=True)[:top_n_properties]
    nodes = [
        {
            "id": item_id,
            "label": item_id,
    ...
    for prop_index, (prop_name, values) in enumerate(sorted_props):
        prop_id = f"prop-{prop_index}"
        nodes.append(
            {
                "id": prop_id,
                "label": prop_name,
        ...
        edges.append({"from": item_id, "to": prop_id})

        for value_index, entry in enumerate(values[:max_values_per_property]):
            val_id = f"val-{prop_index}-{value_index}"
            node = {
                "id": val_id,
                "label": entry["label"],
                "shape": "ellipse",
                "color": "#1d4ed8",
                "font": {"color": "#ffffff"},
                "title": entry.get("url", entry["label"]),
            }
            if entry.get("url"):
                node["url"] = entry["url"]
            nodes.append(node)
            edges.append({"from": prop_id, "to": val_id})

    graph_html = f"""
<div class="wd-network-wrap">
  <div id="wd-network" style="height:480px; width:100%;"></div>
  <div class="wd-network-note">Click a linked value node to open the corresponding Wikidata item.</div>
</div>
...
(function() {{
  ...
  const container = document.getElementById("wd-network");
  ...
  const options = {{
    ...
    nodes: {{ borderWidth: 2, shapeProperties: {{ borderRadius: 10 }} }},
    edges: {{ smooth: {{ type: 'dynamic' }}, arrows: {{ to: false }} }},
  }};
...
<style>
.wd-network-wrap {{ margin-bottom: 1rem; border: 1px solid #dbe4ee; border-radius: 14px; overflow: hidden; box-shadow: 0 8px 24px rgba(15,23,42,.08); }}
.wd-network-note {{ font-size: 0.9rem; color: #334155; padding: 0.75rem 1rem; background: #f8fafc; }}
</style>
"""
    return graph_html
